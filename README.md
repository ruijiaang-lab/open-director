...
- 本地场景状态持久化
- 运镜路径：机位视角按 K 记录节点，导演视角拖拽节点 / 手柄编辑空间路径
- 时间轴面板：播放 / 暂停 / 停止、拖拽节点调整时间、scrub 即时预览
- 贝塞尔曲线段（转为曲线 / 转回直线，3D 手柄拖拽塑形）、缓动（匀速 / 缓入 / 缓出 / 缓入缓出）、段末停顿
- 多机位运镜，机位视角自动隔离路径 / 节点 / 手柄等编辑元素
- 运镜参考视频导出（浏览器原生录制，Chrome 产出 webm，Safari 产出 mp4）
...
- 底部运镜时间轴：播放运镜路径，点选段改时长 / 缓动 / 停顿 / 曲线，拖拽节点字母改时间
...
- `Ctrl/Cmd + V`：粘贴复制对象
- `Ctrl/Cmd + Z`：撤销最近一次操作
- `Delete / Backspace`：删除当前选中对象（选中路径手柄时为恢复直线手柄）
- `Space`：导演视角 ↔ 机位视角
- `K`：机位视角掌镜时保存当前视角（需先点击画面进入掌镜模式）：选中的节点直接更新；未选中时，离已有节点很近（1m 内且朝向接近）视为重调该镜头并更新它，否则新建节点
- `WASD / Q / E`：掌镜移动（`Shift` 加速），`Esc` 释放鼠标
